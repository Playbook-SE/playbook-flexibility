/* ============================================================
   FlexReady microsite — shared header & footer component.
   Edit the navbar / footer ONCE here; every page that includes
   <script src="components.js"></script> gets the update.

   HOW TO USE in each page:
   1. Remove the existing <nav class="navbar">...</nav> block and
      replace it with:   <div id="site-header"></div>
   2. Remove the existing <footer ...>...</footer> block and
      replace it with:   <div id="site-footer"></div>
   3. Just before </body>, add:
      <script src="components.js"></script>

   The active tab is detected automatically from the filename.
   Child pages map to their parent tab:
     persona.html / onboarding.html  -> Customer
     run-sla.html                    -> Go-to-Market
   ============================================================ */
(function () {
  // ── VERSION EXTERNE (playbook-se.github.io) ──────────────────────────────
  // All playbook nav links point back to the internal SE GitHub Pages site.
  // Only persona.html stays on this external host (that's where the AI chat lives).
  var BASE         = 'https://pages.github.schneider-electric.com/SESA565856/flexibility-playbook/';
  var SELF_PERSONA = 'https://playbook-se.github.io/playbook-flexibility/persona.html';
  // ─────────────────────────────────────────────────────────────────────────
  // Brand + footer SVG logos (official Schneider Electric marks)
  var BRAND_LOGO = `<svg class="brand-logo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 218 64">
  <path fill="#009530" d="M24.7 9.47c-4.95-2.18-7.87-3.1-10.8-3.1-3.1 0-5.12 1.1-5.12 2.74 0 5.1 17.56 3.64 17.56 15.48 0 6.56-5.49 10.38-13.17 10.38-6.04 0-8.96-1.64-12.44-3.28v-7.28c4.94 3.28 7.87 4.37 11.7 4.37 3.3 0 5.13-1.1 5.13-3.1C17.56 20.04 0 22.22 0 9.84 0 4 5.12 0 12.99 0c3.84 0 7.31.73 11.7 2.73v6.74Zm24.5 24.04a21.55 21.55 0 0 1-7.86 1.64c-8.23 0-13.36-4.73-13.36-11.84 0-7.1 5.5-12.02 13.17-12.02 2.38 0 5.3.55 7.87 1.46v5.46a12.43 12.43 0 0 0-5.85-1.64c-4.4 0-7.14 2.55-7.14 6.56 0 4 2.75 6.74 6.95 6.74 1.83 0 3.3-.36 6.59-1.64v5.28m67.49-22.4c-7.13 0-12.25 5.1-12.25 12.02 0 7.1 5.3 11.84 13.35 11.84 1.83 0 6.22 0 10.24-3.1v-4.36c-3.47 2.36-5.67 3.27-8.6 3.27-4.56 0-7.3-2.36-7.67-6.37h16.64c.55-8.56-4.94-13.3-11.7-13.3Zm-4.39 9.47c.37-3.28 2.2-5.1 4.94-5.1 2.75 0 4.76 1.82 4.94 5.1h-9.88Zm26.52-9.1h-7.32V34.6h7.32V11.48ZM159.3.55v12.02a14.02 14.02 0 0 0-6.04-1.46c-6.59 0-11.16 4.92-11.16 12.02 0 6.92 4.57 12.02 10.8 12.02 2.37 0 4.38-.72 6.4-2.36v1.82h7.31V.55h-7.32Zm0 27.87c-1.47 1.27-2.75 1.82-4.4 1.82-3.47 0-5.66-2.74-5.66-6.92 0-4.56 2.19-7.3 5.85-7.3 1.28 0 3.1.56 4.02 1.28v11.12h.18Zm22.69-17.31c-7.13 0-12.25 5.1-12.25 12.02 0 7.1 5.3 11.84 13.35 11.84 1.83 0 6.22 0 10.42-3.1v-4.36c-3.47 2.36-5.67 3.27-8.6 3.27-4.57 0-7.31-2.36-7.68-6.37h16.65c.37-8.56-5.12-13.3-11.89-13.3Zm-4.57 9.47c.36-3.28 2.2-5.1 4.94-5.1s4.75 1.82 5.12 5.1h-10.06Zm27.42-2.36c2.2-4.74 4.76-7.1 7.5-7.1 1.47 0 2.75.54 4.76 2.18l-2.01 6.37c-1.83-1.27-3.11-1.64-4.4-1.64-2.74 0-4.38 2.55-6.03 6.74V34.8h-7.31V11.48h7.31v6.74m-65.11-16.4c.73 1.82-.55 4.37-2.75 5.65-2.2 1.27-4.57.9-5.12-.92-.73-1.82.55-4.37 2.75-5.64 2-1.28 4.38-.91 5.12.9Zm-64.2 18.22c0-6.2-4.2-8.74-8.41-8.74-2.93 0-5.3 1.27-7.32 3.82h-.18V.73h-7.32V34.8h7.32V20.22c1.64-2.37 3.1-3.46 4.93-3.46 2.2 0 3.85 1.64 3.85 5.1v8.2a22.9 22.9 0 0 1 7.31-2.37v-7.65m18.12-8.93c-2.93 0-5.3 1.1-7.5 3.83v-3.28h-7.32v15.66c2.56.19 5.67 1.46 7.32 3.28V19.85c1.83-2.91 3.3-3.82 5.12-3.82 2.01 0 3.66 1.27 3.66 4.55v14.03h7.32V20.04c-.19-6.92-5.13-8.93-8.6-8.93Zm12.45 30.06h-8.6v6h8.24v2.56H97.5v6.19h8.78v2.55H94.76V38.62h11.34v2.55Zm9.3-2.55h-2.55v19.85h2.56V38.62Zm9.33 13.66h9.14c0-4.92-2.2-7.1-5.67-7.1-3.66 0-6.03 2.72-6.03 6.55 0 3.64 2.2 6.92 6.4 6.92a9.9 9.9 0 0 0 5.3-1.46v-2.55a7.81 7.81 0 0 1-4.57 1.64c-2.74 0-4.2-1.27-4.57-4Zm3.65-4.92c1.83 0 2.93 1.27 3.11 3.28h-6.58c.18-2 1.46-3.28 3.47-3.28Zm22.52 1.46a5.56 5.56 0 0 0-3.67-1.28c-2.37 0-4.02 1.82-4.02 4.55 0 2.74 1.83 4.38 4.39 4.38a9 9 0 0 0 3.47-.91v2.55a12 12 0 0 1-3.84.72 6.33 6.33 0 0 1-6.58-6.55c0-4.38 2.56-7.1 6.4-7.1 1.46 0 2.56.36 3.84.9v2.74Zm11.87-3.47h4.21v2.37h-4.2v6.74c0 1.46 1.1 2 1.82 2a6.8 6.8 0 0 0 2.93-.9v2.36c-.91.55-2.38.91-3.11.91-2.74 0-4.2-1.82-4.2-4.19v-6.92h-2.2v-.18l4.94-4.74v2.55m13.73 0v3.1c1.28-2.37 2.56-3.28 3.84-3.28 1.1 0 2.01.55 3.11 1.46l-1.46 2.19c-.74-.73-1.83-1.28-2.38-1.28-1.83 0-3.11 1.82-3.11 4v6.93h-2.56V45.35h2.56Zm33.65 3.47a5.56 5.56 0 0 0-3.66-1.28c-2.38 0-4.03 1.82-4.03 4.55 0 2.74 1.83 4.38 4.4 4.38a9.1 9.1 0 0 0 3.47-.91v2.55a12 12 0 0 1-3.84.72 6.33 6.33 0 0 1-6.59-6.55c0-4.38 2.56-7.1 6.4-7.1 1.47 0 2.56.36 3.84.9v2.74Zm-17.2-3.46h-2.56v13.11h2.56V45.36Zm.18-4.74c.18.54-.18 1.46-.91 2-.74.55-1.65.37-1.83-.36-.19-.73.18-1.46.91-2 .73-.37 1.46-.19 1.83.36ZM79 47.18l.74-2.92h7.13c.91-4.19.18-8.01-2.38-10.38-5.12-5.1-15.73-3.28-23.6 4.2-1.27 1.08-2.19 2.36-3.29 3.63H62l-.92 2.92h-5.12c-.55.9-.92 1.82-1.28 2.73h6.03l-.91 2.91h-6.04c-1.1 4.38-.36 8.38 2.2 10.93 4.94 4.92 15.55 3.28 23.6-4.37a18.42 18.42 0 0 0 3.47-4.19h-5.49l.92-2.91h6.4c.55-.91.91-1.82 1.28-2.73H79v.18Zm-1.64-5.47c-.37 0-.73 0-.92.37 0 0-.18.18-.18.36l-2.2 8.75c-.54 3.1-4.38 6.19-9.32 6.19h-6.95l1.28-4.55h4.39c.36 0 .73-.19 1.1-.55.18-.18.18-.37.18-.55l1.83-7.65c.55-3.1 3.84-6.56 8.78-6.56h6.95l-.92 4.2h-4.02Z" />
</svg>`;
  var FOOTER_LOGO = `<svg class="footer-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 218 64">
  <path fill="#fff" d="M24.7 9.47c-4.95-2.18-7.87-3.1-10.8-3.1-3.1 0-5.12 1.1-5.12 2.74 0 5.1 17.56 3.64 17.56 15.48 0 6.56-5.49 10.38-13.17 10.38-6.04 0-8.96-1.64-12.44-3.28v-7.28c4.94 3.28 7.87 4.37 11.7 4.37 3.3 0 5.13-1.1 5.13-3.1C17.56 20.04 0 22.22 0 9.84 0 4 5.12 0 12.99 0c3.84 0 7.31.73 11.7 2.73v6.74Zm24.5 24.04a21.55 21.55 0 0 1-7.86 1.64c-8.23 0-13.36-4.73-13.36-11.84 0-7.1 5.5-12.02 13.17-12.02 2.38 0 5.3.55 7.87 1.46v5.46a12.43 12.43 0 0 0-5.85-1.64c-4.4 0-7.14 2.55-7.14 6.56 0 4 2.75 6.74 6.95 6.74 1.83 0 3.3-.36 6.59-1.64v5.28m67.49-22.4c-7.13 0-12.25 5.1-12.25 12.02 0 7.1 5.3 11.84 13.35 11.84 1.83 0 6.22 0 10.24-3.1v-4.36c-3.47 2.36-5.67 3.27-8.6 3.27-4.56 0-7.3-2.36-7.67-6.37h16.64c.55-8.56-4.94-13.3-11.7-13.3Zm-4.39 9.47c.37-3.28 2.2-5.1 4.94-5.1 2.75 0 4.76 1.82 4.94 5.1h-9.88Zm26.52-9.1h-7.32V34.6h7.32V11.48ZM159.3.55v12.02a14.02 14.02 0 0 0-6.04-1.46c-6.59 0-11.16 4.92-11.16 12.02 0 6.92 4.57 12.02 10.8 12.02 2.37 0 4.38-.72 6.4-2.36v1.82h7.31V.55h-7.32Zm0 27.87c-1.47 1.27-2.75 1.82-4.4 1.82-3.47 0-5.66-2.74-5.66-6.92 0-4.56 2.19-7.3 5.85-7.3 1.28 0 3.1.56 4.02 1.28v11.12h.18Zm22.69-17.31c-7.13 0-12.25 5.1-12.25 12.02 0 7.1 5.3 11.84 13.35 11.84 1.83 0 6.22 0 10.42-3.1v-4.36c-3.47 2.36-5.67 3.27-8.6 3.27-4.57 0-7.31-2.36-7.68-6.37h16.65c.37-8.56-5.12-13.3-11.89-13.3Zm-4.57 9.47c.36-3.28 2.2-5.1 4.94-5.1s4.75 1.82 5.12 5.1h-10.06Zm27.42-2.36c2.2-4.74 4.76-7.1 7.5-7.1 1.47 0 2.75.54 4.76 2.18l-2.01 6.37c-1.83-1.27-3.11-1.64-4.4-1.64-2.74 0-4.38 2.55-6.03 6.74V34.8h-7.31V11.48h7.31v6.74m-65.11-16.4c.73 1.82-.55 4.37-2.75 5.65-2.2 1.27-4.57.9-5.12-.92-.73-1.82.55-4.37 2.75-5.64 2-1.28 4.38-.91 5.12.9Zm-64.2 18.22c0-6.2-4.2-8.74-8.41-8.74-2.93 0-5.3 1.27-7.32 3.82h-.18V.73h-7.32V34.8h7.32V20.22c1.64-2.37 3.1-3.46 4.93-3.46 2.2 0 3.85 1.64 3.85 5.1v8.2a22.9 22.9 0 0 1 7.31-2.37v-7.65m18.12-8.93c-2.93 0-5.3 1.1-7.5 3.83v-3.28h-7.32v15.66c2.56.19 5.67 1.46 7.32 3.28V19.85c1.83-2.91 3.3-3.82 5.12-3.82 2.01 0 3.66 1.27 3.66 4.55v14.03h7.32V20.04c-.19-6.92-5.13-8.93-8.6-8.93Zm12.45 30.06h-8.6v6h8.24v2.56H97.5v6.19h8.78v2.55H94.76V38.62h11.34v2.55Zm9.3-2.55h-2.55v19.85h2.56V38.62Zm9.33 13.66h9.14c0-4.92-2.2-7.1-5.67-7.1-3.66 0-6.03 2.72-6.03 6.55 0 3.64 2.2 6.92 6.4 6.92a9.9 9.9 0 0 0 5.3-1.46v-2.55a7.81 7.81 0 0 1-4.57 1.64c-2.74 0-4.2-1.27-4.57-4Zm3.65-4.92c1.83 0 2.93 1.27 3.11 3.28h-6.58c.18-2 1.46-3.28 3.47-3.28Zm22.52 1.46a5.56 5.56 0 0 0-3.67-1.28c-2.37 0-4.02 1.82-4.02 4.55 0 2.74 1.83 4.38 4.39 4.38a9 9 0 0 0 3.47-.91v2.55a12 12 0 0 1-3.84.72 6.33 6.33 0 0 1-6.58-6.55c0-4.38 2.56-7.1 6.4-7.1 1.46 0 2.56.36 3.84.9v2.74Zm11.87-3.47h4.21v2.37h-4.2v6.74c0 1.46 1.1 2 1.82 2a6.8 6.8 0 0 0 2.93-.9v2.36c-.91.55-2.38.91-3.11.91-2.74 0-4.2-1.82-4.2-4.19v-6.92h-2.2v-.18l4.94-4.74v2.55m13.73 0v3.1c1.28-2.37 2.56-3.28 3.84-3.28 1.1 0 2.01.55 3.11 1.46l-1.46 2.19c-.74-.73-1.83-1.28-2.38-1.28-1.83 0-3.11 1.82-3.11 4v6.93h-2.56V45.35h2.56Zm33.65 3.47a5.56 5.56 0 0 0-3.66-1.28c-2.38 0-4.03 1.82-4.03 4.55 0 2.74 1.83 4.38 4.4 4.38a9.1 9.1 0 0 0 3.47-.91v2.55a12 12 0 0 1-3.84.72 6.33 6.33 0 0 1-6.59-6.55c0-4.38 2.56-7.1 6.4-7.1 1.47 0 2.56.36 3.84.9v2.74Zm-17.2-3.46h-2.56v13.11h2.56V45.36Zm.18-4.74c.18.54-.18 1.46-.91 2-.74.55-1.65.37-1.83-.36-.19-.73.18-1.46.91-2 .73-.37 1.46-.19 1.83.36ZM79 47.18l.74-2.92h7.13c.91-4.19.18-8.01-2.38-10.38-5.12-5.1-15.73-3.28-23.6 4.2-1.27 1.08-2.19 2.36-3.29 3.63H62l-.92 2.92h-5.12c-.55.9-.92 1.82-1.28 2.73h6.03l-.91 2.91h-6.04c-1.1 4.38-.36 8.38 2.2 10.93 4.94 4.92 15.55 3.28 23.6-4.37a18.42 18.42 0 0 0 3.47-4.19h-5.49l.92-2.91h6.4c.55-.91.91-1.82 1.28-2.73H79v.18Zm-1.64-5.47c-.37 0-.73 0-.92.37 0 0-.18.18-.18.36l-2.2 8.75c-.54 3.1-4.38 6.19-9.32 6.19h-6.95l1.28-4.55h4.39c.36 0 .73-.19 1.1-.55.18-.18.18-.37.18-.55l1.83-7.65c.55-3.1 3.84-6.56 8.78-6.56h6.95l-.92 4.2h-4.02Z" />
</svg>`;

  // Which page are we on? (filename without path)
  var path = (location.pathname.split('/').pop() || 'presentation.html').toLowerCase();
  if (path === '' || path === 'index.html') path = 'presentation.html';

  // Map each page to the nav tab that should be marked active.
  // Child pages activate their parent dropdown tab.
  var ACTIVE = {
    'presentation.html': 'presentation',
    'market.html':       'market',
    'market-trends.html':'market',
    'customer.html':     'customer',
    'persona.html':      'customer',
    'onboarding.html':   'customer',
    'offer.html':        'offer',
    'gtm.html':          'gtm',
    'run-sla.html':      'gtm',
    'business-model.html':'business',
    'roadmap.html':      'roadmap',
    'glossary.html':     'glossary'
  };
  var active = ACTIVE[path] || '';

  // Helper: returns 'nav-link active' or 'nav-link', and renders a
  // plain <span> for the active simple tab (matches original markup).
  function tab(key, label, href) {
    if (active === key) {
      return '<span class="nav-link active">' + label + '</span>';
    }
    return '<a class="nav-link" href="' + href + '">' + label + '</a>';
  }
  // For dropdown parent tabs: the <a> keeps its href but gains "active".
  function ddParent(key, label, href) {
    var cls = (active === key) ? 'nav-link active' : 'nav-link';
    return '<a class="' + cls + '" href="' + href + '">' + label +
           ' <span class="nav-caret">\u25BC</span></a>';
  }

  var HEADER =
    '<nav class="navbar">' +
      '<a class="brand" href="' + BASE + 'presentation.html">' + BRAND_LOGO + '</a>' +
      '<div class="nav-sep"></div>' +
      '<span class="nav-product">Exchange \u00B7 EcoStruxure FlexReady API</span>' +
      '<div class="nav-links">' +
        tab('presentation', 'Presentation', BASE + 'presentation.html') +
        '<div class="nav-item">' +
          ddParent('market', 'Market', BASE + 'market.html') +
          '<div class="nav-dropdown">' +
            '<a href="' + BASE + 'market.html">Market / Opportunity<span class="nav-dd-sub">Situation, segments, problems</span></a>' +
            '<div class="nav-dd-sep"></div>' +
            '<a href="' + BASE + 'market-trends.html">Market Trends<span class="nav-dd-sub">Drivers, DERs, AI</span></a>' +
          '</div>' +
        '</div>' +
        '<div class="nav-item">' +
          ddParent('customer', 'Customer', BASE + 'customer.html') +
          '<div class="nav-dropdown">' +
            '<a href="' + BASE + 'customer.html">Customer overview<span class="nav-dd-sub">Focus &amp; experience</span></a>' +
            '<div class="nav-dd-sep"></div>' +
            '<a href="' + SELF_PERSONA + '">Personas<span class="nav-dd-sub">6 profiles &amp; data flows</span></a>' +
            '<a href="' + BASE + 'onboarding.html">Onboarding<span class="nav-dd-sub">Onboarding, 13 steps</span></a>' +
            '<a href="' + BASE + 'customer.html#kpis">Expected outcomes &amp; KPIs<span class="nav-dd-sub">Expected outcomes</span></a>' +
          '</div>' +
        '</div>' +
        tab('offer', 'Offer Positioning', BASE + 'offer.html') +
        '<div class="nav-item">' +
          ddParent('gtm', 'Go-to-Market', BASE + 'gtm.html') +
          '<div class="nav-dropdown">' +
            '<a href="' + BASE + 'gtm.html">Go-to-Market overview<span class="nav-dd-sub">Execution &amp; delivery</span></a>' +
            '<div class="nav-dd-sep"></div>' +
            '<a href="' + BASE + 'run-sla.html">Delivery &amp; run process<span class="nav-dd-sub">Support, SLA, release</span></a>' +
          '</div>' +
        '</div>' +
        tab('business', 'Business Model', BASE + 'business-model.html') +
        tab('roadmap', 'Roadmap &amp; Risk', BASE + 'roadmap.html') +
        tab('glossary', 'Glossary', BASE + 'glossary.html') +
        '<a class="nav-link" href="https://exchange.se.com/develop/products/864231/ecostruxure-energy-interactive-flexready-api" target="_blank">API docs</a>' +
        '<a class="nav-link" href="https://exchange.se.com/devportal/?api=flexibility-api-spec-v1&amp;id=864231" target="_blank">Dev portal</a>' +
      '</div>' +
    '</nav>';

  var FOOTER =
    '<footer class="footer">' +
      '<div class="footer-brand">' +
        '<div class="footer-logo">' + FOOTER_LOGO + '</div>' +
      '</div>' +
      '<div class="footer-links">' +
        '<a class="footer-link" href="https://exchange.se.com" target="_blank">exchange.se.com</a>' +
        '<a class="footer-link" href="https://exchange.se.com/develop/products/864231" target="_blank">FlexReady API</a>' +
        '<span class="footer-link">EcoStruxure Platform</span>' +
        '<span class="footer-link">se.com</span>' +
      '</div>' +
      '<div class="footer-copy">\u00A92026 Schneider Electric. All Rights Reserved.</div>' +
    '</footer>';

  function inject() {
    var h = document.getElementById('site-header');
    var f = document.getElementById('site-footer');
    if (h) h.outerHTML = HEADER;
    if (f) f.outerHTML = FOOTER;
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', inject);
  } else {
    inject();
  }
})();
